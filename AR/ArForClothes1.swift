...
struct ArForClothes1: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: ARView, context: Context) {
        if ARBodyTrackingConfiguration.isSupported { // 檢查是否支援 ARBodyTrackingConfiguration
            ...
            uiView.scene.addAnchor(characterAnchor) // 將角色錨點實體加入 ARView 的場景中
                ...
                cancellable = Entity.loadBodyTrackedAsync(named: "character/robot_dressed").sink(
    ...
    class Coordinator: NSObject, ARSessionDelegate {
        let parent: ArForClothes1
        
        init(_ parent: ArForClothes1) {
            self.parent = parent
        }
...
struct ArForClothes1View_Previews: PreviewProvider {
